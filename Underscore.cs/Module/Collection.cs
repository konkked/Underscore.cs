...
namespace Underscore.Module
{
        ...
        IUnZipComponent
	{
	    ...
	    private readonly IMapReduceComponent _mapReduce;
        ...
        {
            return _mapReduce.Map(collection, transform);
        }

        public U Reduce<T, U>(IEnumerable<T> collection, Func<U, T, U> reducer)
        {
            return _mapReduce.Reduce(collection, reducer);
        }

        public U Reduce<T, U>(IEnumerable<T> collection, U seed, Func<U, T, U> reducer)
        {
            return _mapReduce.Reduce(collection, seed, reducer);
        }
        #endregion
